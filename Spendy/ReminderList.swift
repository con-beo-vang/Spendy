...
//        let item = ReminderItem(category: category, reminderTime: date, UUID: NSUUID().UUIDString)
        ...
        notification.alertBody = "Good day! Did you spend $\(item.predictedAmount) on \(item.category!.name)?" // text that will be displayed in the notification
        ...
        notification.userInfo = ["categoryId": categoryId!, "predictedAmount": item.predictedAmount, "UUID": item.UUID] // assign a unique identifier to the notification so that we can retrieve it later
